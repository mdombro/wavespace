...
"""
Capture discrete, trigger-aligned PDM measurement blocks from the Pico over USB CDC,
the vendor-specific USB bulk endpoint, or Wi-Fi UDP. Each measurement consists of a
16-bit index followed by a fixed-duration slice of the raw PDM bitstream. The samples
are stored in a NumPy .npz archive with two arrays: `index` (uint16) and `data`
(uint8, shape=(measurements, payload_bytes)).

When talking to the device over USB this helper understands the interactive debug CLI
that the firmware now exposes. It will automatically:
...
In Wi-Fi mode the script listens for fixed-size UDP payloads from the Pico and captures
each measurement.
"""
...
DEFAULT_PDM_CLOCK_HZ = 2_048_000
DEFAULT_MEASUREMENT_WINDOW_MS = 20
INDEX_FIELD_BYTES = 2
CLI_READ_WINDOW = 1.0
...
def measurement_payload_bytes(clock_hz: int, window_ms: int) -> int:
    if clock_hz <= 0:
        raise ValueError("PDM clock must be greater than zero.")
    if window_ms <= 0:
        raise ValueError("Measurement window must be greater than zero.")

    bits = clock_hz * window_ms
    bytes_per_window = (bits + 7999) // 8000
    if bytes_per_window <= 0:
        raise ValueError("Computed payload length was zero; check --clock and --window-ms.")
    return bytes_per_window


def append_measurement_packet(
    packet: bytes,
    payload_bytes: int,
    *,
    indices: List[int],
    ...
    expected = payload_bytes + INDEX_FIELD_BYTES
    if len(packet) != expected:
        raise ValueError(
            f"Unexpected measurement size: received {len(packet)} byte(s), expected {expected}. "
            "Ensure the --clock/--window-ms arguments match the firmware configuration."
        )

    index = int.from_bytes(packet[:INDEX_FIELD_BYTES], "little", signed=False)
    payload = packet[INDEX_FIELD_BYTES:]
    if len(payload) != payload_bytes:
        raise ValueError("Measurement payload length mismatch while parsing packet.")

    indices.append(index)
    payloads.append(payload)


def write_measurements_npz(
    output_path: Path,
    ...
    if len(indices) != len(payloads):
        raise ValueError("Measurement index and payload counts diverged.")

    measurement_count = len(indices)
    index_array = np.asarray(indices, dtype=np.uint16)
    data_array = np.empty((measurement_count, payload_bytes), dtype=np.uint8)

    for row, payload in enumerate(payloads):
        view = np.frombuffer(payload, dtype=np.uint8)
        if view.size != payload_bytes:
            raise ValueError(
                f"Measurement {row} has {view.size} bytes but expected {payload_bytes}; capture may be corrupt."
            )
        data_array[row] = view

    np.savez(output_path, index=index_array, data=data_array)
...
def capture_stream_usb(
    ...
    *,
    ...
    with serial.Serial(port=port, baudrate=115200, timeout=serial_timeout) as device:
        ...
        try:
            while blocks is None or block_count < blocks:
                data = read_exact(device, packet_bytes)
                append_measurement_packet(data, payload_bytes, indices=indices, payloads=payloads)
                block_count += 1
    ...
    print(
        f"Captured {len(indices)} measurement(s) ({len(indices) * packet_bytes} bytes) over USB CDC.",
        file=sys.stderr,
    )
...
def capture_stream_wifi(
    ...
    *,
    ...
    print(
        f"Captured {len(indices)} measurement(s) ({len(indices) * packet_bytes} bytes) over Wi-Fi UDP.",
        file=sys.stderr,
    )
    return indices, payloads
    ...
    *,
    ...
    try:
        ...
        try:
            while blocks is None or block_count < blocks:
                try:
                    data = endpoint.read(packet_bytes, timeout=usb_timeout)
                except usb.core.USBError as exc:  # type: ignore[attr-defined]
                ...
                append_measurement_packet(bytes(data), payload_bytes, indices=indices, payloads=payloads)
                block_count += 1
    ...
    print(
        f"Captured {len(indices)} measurement(s) ({len(indices) * packet_bytes} bytes) over the USB vendor endpoint.",
        file=sys.stderr,
    )
    return indices, payloads
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Capture discrete, trigger-aligned PDM measurement blocks from the Pico over USB (CDC/vendor) or Wi-Fi."
    )
    parser.add_argument(
        "source",
        nargs="?",
    ...
    parser.add_argument("output", help="Destination .npz file path for the captured measurements")
    parser.add_argument(
        "--transport",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override the expected measurement packet size (index + payload) in bytes. "
            "By default this is derived from --clock and --window-ms."
        ),
    )
    parser.add_argument(
        "--blocks",
        type=int,
        default=None,
        help="Optional limit for the number of measurements to capture (default: capture until interrupted).",
    )
    parser.add_argument(
        "--timeout",
        type=float,
    ...
    parser.add_argument(
        "--window-ms",
        type=int,
        default=DEFAULT_MEASUREMENT_WINDOW_MS,
        help=(
            "Measurement window in milliseconds. Must match the firmware's setting (default: "
            f"{DEFAULT_MEASUREMENT_WINDOW_MS})."
        ),
    )
    parser.add_argument(
        "--keep-stats",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.block_bytes is not None and args.block_bytes <= 0:
        print("Block size must be positive", file=sys.stderr)
        return 2
    if args.window_ms <= 0:
        print("Measurement window (--window-ms) must be positive", file=sys.stderr)
        return 2

    clock_hz = args.clock if args.clock is not None else DEFAULT_PDM_CLOCK_HZ
    try:
        payload_bytes = measurement_payload_bytes(clock_hz, args.window_ms)
    except ValueError as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 2

    packet_bytes = payload_bytes + INDEX_FIELD_BYTES
    if args.block_bytes is not None:
        if args.block_bytes <= INDEX_FIELD_BYTES:
            print("Block size must exceed the 2-byte measurement index.", file=sys.stderr)
            return 2
        packet_bytes = args.block_bytes
        payload_bytes = packet_bytes - INDEX_FIELD_BYTES

    measurement_path = Path(args.output)
    indices: List[int] = []
    payloads: List[bytes] = []
    ...
    if args.transport == "usb":
        ...
        try:
            indices, payloads = capture_stream_usb(
                args.source,
                packet_bytes,
                payload_bytes,
                args.blocks,
        ...
        try:
            indices, payloads = capture_stream_usb_vendor(
                args.source,
                packet_bytes,
                payload_bytes,
                args.blocks,
        ...
        if args.keep_stats or args.keep_dumps or args.no_restore:
            print(
                "Warning: --keep-stats/--keep-dumps/--no-restore are ignored for Wi-Fi captures.",
                file=sys.stderr,
            )
        if args.clock is not None:
            print(
                "Note: --clock is not sent to the device when using Wi-Fi; it is only used to size incoming packets.",
                file=sys.stderr,
            )

        try:
            indices, payloads = capture_stream_wifi(
                args.wifi_bind,
                args.wifi_port,
                packet_bytes,
                payload_bytes,
    ...
    write_measurements_npz(measurement_path, indices, payloads, payload_bytes)
    print(f"Wrote {len(indices)} measurement(s) to {measurement_path}", file=sys.stderr)
    return 0
